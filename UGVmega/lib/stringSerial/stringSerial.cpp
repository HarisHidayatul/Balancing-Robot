...
char* stringSerial::getdata(float value, int codeSensor){
    initHeader();
    /*
    if((codeSensor == lat)||(codeSensor == lng)){
        appendChar(getStr());
        ...
        data_char[1] = unitNumb;
        appendChar(convert2dString(codeSensor));
        appendChar(convert10dString(value));
        processFCS();
    int b = countString(data_char);
    data_char[b] = delimiter;
    return (char *)data_char;
...
void stringSerial::accSensor(int codeSensor){
    ...
    case compass:
        ...
        break;
    case lattt:
        appendChar(convert10dString(accSensor::latitude));
        break;
    case lnggg:
        appendChar(convert10dString(accSensor::longtitude));
        break;
    default:
        break;
    }
...
char* stringSerial::convert4dString(int val){
    ...
    if((val > 9999)||(val < -999)){for(int i=0;i<4;i++)data_4d[i]='0';}
    else{
        if(val >= 0){data_4d[0] = (val/1000 % 10) +48;}
        else{data_4d[0] = '-';val = val*(-1);}
